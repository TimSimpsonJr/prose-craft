---
name: init
description: Initialize copydesk on this machine and extract a voice register. Creates the user data directory, copies templates, and walks you through the extraction process to create your first register. Also use this to add a new register later. Invoke via /copydesk:init.
---
...
> Register `<register-name>` is ready at `~/.claude/data/copydesk/registers/<register-name>.md` with these triggers: [list]. Try it now: invoke `/copydesk` with writing that matches one of the trigger contexts.
